 ...
 */
export function findOasOperation(operationId: string | undefined, oasSpec: OasSpec | null): OasOperationInfo | undefined {
  if (!operationId || !oasSpec || !oasSpec.paths) return undefined;

  // Standardize operationId formats for matching
  const normalize = (id: string): string => id.toLowerCase().replace(/^smbdo-/, '').replace(/-/g, '').replace(/_/g, '');
 ...
 */
export function resolveSchemaRef(ref: string, oasSpec: OasSpec | null): Record<string, any> | null {
 ...
 */
export function getSchemaForPath(path: string, oasSpec: OasSpec | null, operationId?: string): Record<string, any> | null {
  ...
  if (operation.verb === 'POST' || operation.verb === 'PUT' || operation.verb === 'PATCH') {
    rootSchema = operation.requestBody?.content?.['application/json']?.schema || null;
  }
  // For GET operations, look for response schema
  else if (operation.verb === 'GET' && operation.responses?.['200']?.content?.['application/json']?.schema) {
  ...
  for (const segment of segments) {
    ...
    if (!isNaN(Number(segment))) {
      ...
      // If it's allOf, oneOf, or anyOf, we might need to merge schemas
        ...
        const schemaList: any[] = currentSchema.allOf || currentSchema.oneOf || currentSchema.anyOf;
        let found = false;
        ...
        for (const subSchema of schemaList) {
          ...
          if (resolvedSchema.properties && resolvedSchema.properties[segment]) {
            // Check if this property is required
            const isRequired = 
              Array.isArray(resolvedSchema.required) && 
              resolvedSchema.required.includes(segment);
            
            currentSchema = resolvedSchema.properties[segment];
 ...
 */
export function extractValidationConstraints(schema: Record<string, any> | null): string {
  if (!schema) return '';
...
export function flattenJsonPaths(
  ...
  const rows: { path: string; value: unknown; schemaInfo?: Record<string, any> }[] = [];
