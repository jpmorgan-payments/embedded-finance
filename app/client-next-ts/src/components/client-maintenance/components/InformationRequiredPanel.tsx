...
export function InformationRequiredPanel({
  ...
  phase: 'before-attestation' | 'after-attestation';
  ...
  const isBeforeAttestation = phase === 'before-attestation';
      ...
      <div className="border-b border-gray-200 px-5 py-5 sm:px-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            >
              {isBeforeAttestation
                ? 'Questions before attestation'
                : 'More information required'}
            </h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-gray-700">
              {isBeforeAttestation
                ? 'Questions can be returned before or after attestation. Document requests appear only after attestation.'
                : 'The submitted maintenance request is locked while these later questions and document requests are resolved. Respond within 30 days to avoid automatic termination.'}
            </p>
          </div>
          <Badge
            variant="outline"
      ...
      {newParties.length > 0 ? (
        <div className="border-b border-amber-200 bg-amber-50 px-5 py-5 sm:px-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 max-w-3xl text-sm leading-6 text-gray-700">
                  ...
                  approved while these requirements are outstanding. Questions
                  remain client-level when the API response has no party ID.
                </p>
              </div>
            </div>
            <Badge
            ...
            >
              {isBeforeAttestation
                ? 'New party · questions outstanding'
