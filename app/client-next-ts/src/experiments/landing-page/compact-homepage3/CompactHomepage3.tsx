    ...
    <div className="min-h-screen bg-sp-bg">
      ...
      <section className="pb-12 pt-10 sm:pb-16 sm:pt-14 lg:pb-20 lg:pt-16">
        <div className="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
          {/* Badge */}
          <div
            className={`mb-6 inline-flex items-center gap-3 rounded-full border border-sp-border bg-white px-4 py-2 text-sm transition-all duration-700 ${
          ...
          {/* Headline */}
          <h1
            className={`mb-6 text-4xl font-bold leading-tight text-jpm-gray-900 transition-all delay-100 duration-700 sm:text-5xl lg:text-6xl ${
              isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
            }`}
          >
            Embedded Finance
          ...
          {/* Subtitle */}
          <p
            className={`mx-auto mb-10 max-w-2xl text-lg leading-relaxed text-jpm-gray transition-all delay-200 duration-700 sm:text-xl ${
              isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
            }`}
          >
          ...
          <div
            className={`mb-10 grid gap-4 transition-all delay-300 duration-700 sm:grid-cols-2 ${
              isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
            }`}
          >
            {/* Install Command */}
            <div className="group relative overflow-hidden rounded-lg border border-sp-border bg-jpm-gray-900 p-4 text-left">
              <div className="mb-2 text-xs font-medium uppercase tracking-wider text-jpm-gray-400">
                Install
              </div>
            ...
            {/* Usage Example */}
            <div className="overflow-hidden rounded-lg border border-sp-border bg-jpm-gray-900 p-4 text-left">
              <div className="mb-2 text-xs font-medium uppercase tracking-wider text-jpm-gray-400">
                Usage
              </div>
              <code className="block font-mono text-sm text-jpm-gray-100">
      ...
      <section className="bg-white py-12 sm:py-16">
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          <h2
            className={`mb-10 text-center text-2xl font-bold text-jpm-gray-900 transition-all delay-500 duration-700 sm:text-3xl ${
              isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
            }`}
          >
          ...
          <div className="grid gap-6 sm:grid-cols-3">
            {actionCards.map((card, index) => (
              <Link
                key={card.id}
                to={card.link}
                className={`group rounded-xl border-2 border-sp-border bg-white p-6 text-center transition-all duration-500 hover:-translate-y-1 hover:border-sp-brand hover:shadow-lg ${
                  isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
                }`}
                style={{ transitionDelay: `${550 + index * 100}ms` }}
              >
                <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-sp-accent text-sp-brand transition-transform duration-300 group-hover:scale-110">
                  {card.icon}
                </div>
                <h3 className="mb-2 text-lg font-bold text-jpm-gray-900">
                  {card.title}
                </h3>
                <p className="mb-4 text-sm text-jpm-gray">
                  {card.description}
                </p>
      ...
      <section className="bg-sp-bg py-12 sm:py-16">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          <h2
            className={`mb-8 text-center text-2xl font-bold text-jpm-gray-900 transition-all delay-700 duration-700 sm:text-3xl ${
              isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
            }`}
          >
          ...
          {/* Tabs */}
          <div
            className={`mb-6 flex justify-center transition-all delay-[750ms] duration-700 ${
              isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
            }`}
          >
          ...
          >
            ...
            <Link
              to="/solutions"
              className="mt-4 inline-flex items-center text-sm font-medium text-sp-brand hover:text-sp-brand-700"
            >
      ...
      {/* Footer Quick Links */}
      <section className="border-t border-sp-border bg-white py-8">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
