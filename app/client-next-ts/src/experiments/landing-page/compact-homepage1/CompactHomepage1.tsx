...
export function CompactHomepage1() {
  ...
  return (
    <div className="min-h-screen">
      ...
      >
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 items-start gap-8 lg:grid-cols-3 lg:gap-12">
            ...
            <div
              className={`transition-all duration-700 ease-out lg:col-span-2 ${
                isVisible
            ...
            >
              <h1 className="mb-6 font-heading text-3xl leading-[1.1] tracking-tight text-sp-brand sm:text-4xl md:text-5xl lg:text-page-hero">
                ...
                <span className="mt-1 block font-bold text-sp-brand">
                  Showcases and Components
                </span>
              </h1>

              <p
                className={`mb-8 max-w-2xl text-lg leading-relaxed text-jpm-blue transition-all delay-100 duration-700 sm:text-xl lg:text-page-body ${
                  isVisible
                    ? 'translate-y-0 opacity-100'
              ...
              <div
                className={`flex flex-col gap-4 transition-all delay-200 duration-700 sm:flex-row sm:flex-wrap sm:items-center ${
                  isVisible
                    ? 'translate-y-0 opacity-100'
                    : 'translate-y-6 opacity-0'
                }`}
              >
                <Link to="/demos" className="group inline-flex">
                  <Button
                    size="lg"
                    className="w-full rounded-page-md border-0 bg-sp-brand px-8 py-4 text-base font-semibold !text-jpm-white shadow-[0_4px_14px_0_rgba(26,123,153,0.3)] transition-all duration-300 hover:-translate-y-0.5 hover:bg-sp-brand-700 hover:shadow-[0_6px_20px_0_rgba(26,123,153,0.4)] sm:w-auto"
                  >
                    <Play className="size-5 shrink-0" />
                    EXPLORE DEMOS
                  </Button>
                </Link>
                <Link to="/documentation" className="group inline-flex">
                  <Button
                    variant="outline"
                    size="lg"
                    className="w-full rounded-page-md border-2 border-sp-brand px-8 py-4 text-base font-semibold text-sp-brand transition-all duration-300 hover:-translate-y-0.5 hover:bg-sp-brand hover:text-jpm-white hover:shadow-lg sm:w-auto"
                  >
                    <BookOpen className="size-5 shrink-0" />
                    VIEW DOCUMENTATION
                  </Button>
                </Link>
                <HeroSkillInstallDialog tone="compact-accent" />
              </div>
            </div>

            {/* Right side - Interactive Infographic with entrance animation */}
            <div
              className={`transition-all delay-300 duration-700 lg:col-span-1 ${
                isVisible
                  ? 'translate-y-0 opacity-100'
                  : 'translate-y-8 opacity-0'
              }`}
      ...
      <section ref={cardsRef} className="relative bg-sp-bg pb-12 pt-12">
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {navigationCards.map((card, index) => (
              ...
              >
                <Card
                  className={`group relative flex h-full min-h-[12rem] cursor-pointer flex-col overflow-hidden rounded-page-md border-2 border-sp-border bg-jpm-white transition-all duration-300 ${
                    hoveredCardId === card.id
                      ? '-translate-y-2 transform border-sp-brand shadow-[0_12px_24px_-8px_rgba(26,123,153,0.25)] ring-2 ring-sp-brand/20'
                      : 'shadow-[0_2px_8px_-2px_rgba(0,0,0,0.08)] hover:border-sp-brand/50 hover:shadow-[0_8px_16px_-4px_rgba(0,0,0,0.12)]'
                  }`}
                  style={{
                ...
                >
                  ...
                  <div
                    className={`absolute inset-0 rounded-page-md bg-gradient-to-r ${card.borderGradient} -z-10 opacity-0 blur-xl transition-opacity duration-300 ${
                      hoveredCardId === card.id ? 'opacity-20' : ''
                    }`}
                  />

                  <CardHeader
                    className={`border-b-2 border-sp-border ${card.accentColor} relative flex-shrink-0 overflow-hidden p-5`}
                  >
                    ...
                    <div className="relative z-10 mb-3 flex items-center justify-between">
                      <div
                        className={`flex h-14 w-14 items-center justify-center rounded-page-md ${card.iconBg} text-sp-brand shadow-[0_2px_8px_-2px_rgba(0,0,0,0.1)] transition-all duration-300 ${
                          hoveredCardId === card.id
                            ? 'scale-110 shadow-[0_4px_12px_-2px_rgba(26,123,153,0.3)] ring-2 ring-sp-brand/30'
                    ...
                    <CardTitle className="relative z-10 text-base font-bold leading-tight text-jpm-gray-900">
                      {card.title}
                    </CardTitle>
                  </CardHeader>
