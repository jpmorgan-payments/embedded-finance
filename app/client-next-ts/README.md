...
- Design tokens (colors, radii, typography scales, etc.) are defined in
  `tailwind.config.js` under `theme.extend`.
- We configure semantic font families in `theme.extend.fontFamily` (e.g.,
  `heading`, `body`, `card`) and auto-generate matching utilities via a small
  Tailwind plugin (`font-heading`, `font-body`, `font-card`).
- Global font is applied via Tailwind base layer using `font-sans` on `html` in
  `src/styles.css`. This ensures Tailwind is the single source of truth for
  fonts.

Example usage in components:

```tsx
<h2 className="font-heading text-page-h2">Heading</h2>
<p className="font-body text-page-body">Body copy</p>
<div className="font-card">Card content</div>
```

Plugin snippet in `tailwind.config.js`:
...
[Layouts documentation](https://tanstack.com/router/latest/docs/framework/react/routing/routing-concepts#layout-routes).
