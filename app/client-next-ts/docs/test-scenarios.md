...
| [`/test-scenario-5`](http://localhost:3000/test-scenario-5) | `test-scenario-5`        | CommerceBridge Marketplace LLC; `Empty` theme; three recommended **NAICS codes** (`455110`, `455211`, `459999`). Selecting a NAICS code serves **eight** marked assessment questions on `GET /clients` `outstanding.questionIds` (four single-select with 10–12 options). Answering them auto-approves the client and opens a wallet dashboard (accounts, linked accounts, recipients, transactions). **Two logins:** `naics-codes-onboarding@demo.test` (NEW → onboarding → questions → dashboard) and `dashboard-approved@demo.test` (APPROVED dashboard immediately). |
