...
| ----------------------------------------------------------- | ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`/test-scenario`](http://localhost:3000/test-scenario)     | `test-scenario`          | Operator 80–shaped LLC client; Salt theme; full login paths including link-bank. Prepopulated Business Type, Business Name, DBA, EIN, Business Description, and Industry fields use `readonlyFields` with `whenPopulated`; Business Address remains editable.                                                                                                                                                                                                                                                                                                               |
...
| [`/test-scenario-3`](http://localhost:3000/test-scenario-3) | `test-scenario-3`        | Health & Benefit Solutions client; **single** link-bank account (ACH + WIRE + RTP); health-sector `priorityIndustryCodes`. Gateway offers **all corp org types** (sole prop, LLC, LLP, GP, LP, C-corp). **Two happy paths:** `happy-path-ptc@demo.test` (`happy-path-ptc` mode, C-corp + Nasdaq PTC in MSW, `enablePubliclyTradedCompanies: true`) and `happy-path@demo.test` (LLC in MSW, no `publiclyTraded`, component flag off). Other logins match `/test-scenario` (docs, linked accounts; no pre-seeded multi-link).                                                                                                                              |
| [`/test-scenario-4`](http://localhost:3000/test-scenario-4) | `test-scenario-4`        | Faster Fulfilment Corp.; `Empty` theme; logistics `priorityIndustryCodes`. Gateway offers **all corp org types** (sole prop, LLC, LLP, GP, LP, C-corp). **Two happy paths:** `happy-path-ptc@demo.test` (`happy-path-ptc` mode, C-corp + Nasdaq PTC in MSW, `enablePubliclyTradedCompanies: true`) and `happy-path@demo.test` (LLC in MSW, no `publiclyTraded`, component flag off). Other logins match `/test-scenario-2` (docs, linked accounts, `3-linked@demo.test`). Same **root chrome** as other test-scenario routes (no Solutions Showcase `LandingHeader`).                                                                                    |
| [`/test-scenario-5`](http://localhost:3000/test-scenario-5) | `test-scenario-5`        | Leap Frog Investments, LLC; `Empty` theme; three recommended **NAICS codes** (`525996`, `525995`, `525991`) for fund management platforms. Selecting a NAICS code serves **23** fund assessment questions on `GET /clients` `outstanding.questionIds`. Answering them auto-approves the client and opens a payments account dashboard (accounts, recipients, transactions — linked accounts suppressed). **Two logins:** `naics-codes-onboarding@demo.test` (NEW → onboarding → questions → dashboard) and `dashboard-approved@demo.test` (APPROVED dashboard immediately). |
| [`/test-scenario-6`](http://localhost:3000/test-scenario-6) | `test-scenario-6`        | Bright Fork Kitchen, LLC (casual dining restaurant chain); `Empty` theme; restaurant NAICS (`722511`, `722513`, `722515`). **One login** with the same Delta-mode shape as Storybook **Sole owner-operator**: Jordan Hale as combined controller / sole owner / authorized user; outstanding sanctions questions `30158`/`30162`; missing operator SSN, birthdate, business and operator address/phone. Host props: `deltaMode.enabled`, `maxPendingFields: 5`, `defaultControllerNotAnOwner: true`, `skipTermsDocumentAcknowledgment`. Linked-account step off.            |
...
| [`src/routes/test-scenario-5.tsx`](../src/routes/test-scenario-5.tsx)                                           | `/test-scenario-5`              |
| [`src/routes/test-scenario-6.tsx`](../src/routes/test-scenario-6.tsx)                                           | `/test-scenario-6`              |
