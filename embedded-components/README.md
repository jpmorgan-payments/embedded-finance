...
- **Delta mode** — a distilled completion path for when the platform already provides most of the user's data
...
| `deltaMode`                                  | `boolean \| { enabled: boolean; maxPendingFields?: number; defaultControllerNotAnOwner?: boolean; reviewSectionsDisplay?: 'collapsible' \| 'requireReview' \| 'expanded' }` | No       | Distilled **delta** completion for pre-created clients whose data your platform already provides — opens straight on **Review & attest** to finish the few remaining fields. Activates only when pending fields ≤ `maxPendingFields` (default `5`). See **`src/core/OnboardingFlow/README.md`** (Delta mode) and **DELTA_MODE_SPEC.md**. |
| `enablePubliclyTradedCompanies`              | `boolean`                                                                                                                                                                   | No       | Enable publicly-traded-company (PTC) onboarding — the gateway asks whether the org is publicly traded or a subsidiary of a PTC and collects ticker + stock exchange. Default `false`. See **PTC** section in **`src/core/OnboardingFlow/README.md`**.                                                                                    |
| `disclosureConfig`                           | `OnboardingDisclosureConfig`                                                                                                                                                | No       | Platform disclosure + attestation config (`platformName`, `productName`, `platformAgreementUrl`, `platformAgreementLabel`). Enables enhanced review-step attestation and FDIC / "not a bank" footer language.                                                                                                                            |
| `showDisclosureFooter`                       | `boolean`                                                                                                                                                                   | No       | Show the regulatory disclosure footer (requires `disclosureConfig`). Default `false`.                                                                                                                                                                                                                                                    |
| `reviewAttestTermsAcknowledgements`          | `ReviewAttestTermsAcknowledgement[]`                                                                                                                                        | No       | Replace the built-in review-step attestation checkboxes with a host-defined list (every item must be checked to submit).                                                                                                                                                                                                                 |
| `showReviewAttestTermsAcknowledgementsIntro` | `boolean`                                                                                                                                                                   | No       | Show the intro line above `reviewAttestTermsAcknowledgements`. Default `false`.                                                                                                                                                                                                                                                          |
| `skipTermsDocumentAcknowledgment`            | `boolean`                                                                                                                                                                   | No       | Enable the terms attestation checkboxes without first requiring the user to open every terms document (links still render). Default `false`.                                                                                                                                                                                             |
| `showLinkAccountStep`                        | `boolean`                                                                                                                                                                   | No       | Show the **Link bank account** step. Use with `linkAccountEnabledStatuses` to control which client statuses allow linking.                                                                                                                                                                                                               |
| `linkAccountEnabledStatuses`                 | `ClientStatus[]`                                                                                                                                                            | No       | Client statuses for which the linked-account step is enabled (takes precedence over the default check; `[]` disables linking for all statuses).                                                                                                                                                                                          |
| `linkAccountStepOptions`                     | `LinkAccountStepOptions`                                                                                                                                                    | No       | Pre-populate / configure the Link bank account step (see the options table in **`src/core/OnboardingFlow/README.md`**).                                                                                                                                                                                                                  |
| `showDownloadChecklist`                      | `boolean`                                                                                                                                                                   | No       | Show the "Download Checklist" button on the Overview screen. Default `false`.                                                                                                                                                                                                                                                            |
| `priorityIndustryCodes`                      | `readonly string[]`                                                                                                                                                         | No       | Host-curated NAICS codes pinned as a "Suggested for your platform" group at the top of the industry combobox.                                                                                                                                                                                                                            |
| `enableIndustrySuggestions`                  | `boolean`                                                                                                                                                                   | No       | Show the "Suggest" button on the Industry step, which recommends NAICS codes from the business description. Falls back to the `NAICS_SUGGESTION_FEATURE_FLAG` localStorage flag when omitted. Default `false`.                                                                                                                           |
| `docUploadMaxFileSizeBytes`                  | `number`                                                                                                                                                                    | No       | Maximum upload size (bytes) for document uploads.                                                                                                                                                                                                                                                                                        |
| `hideSidebar`                                | `boolean`                                                                                                                                                                   | No       | Hide the flow sidebar.                                                                                                                                                                                                                                                                                                                   |
| `flowEntry`                                  | `OnboardingFlowEntry`                                                                                                                                                       | No       | Open the flow at a specific screen after client data loads (intended for Storybook / tests; ignored in doc-upload-only mode).                                                                                                                                                                                                            |
