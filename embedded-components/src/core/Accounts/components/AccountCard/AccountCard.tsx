...
interface AccountBalanceProps {
  balanceData: AccountBalanceResponse | undefined;
...
interface AccountCardViewProps extends AccountBalanceProps {
  account: AccountResponse;
  className?: string;
  hideBorder: boolean;
  Heading: ReturnType<typeof getHeadingTag>;
  ...
  routingEntries: ReturnType<typeof normalizeRoutingInformation>;
  isOpen: boolean;
...
/**
 * Compact-mode balance display
 */
function CompactBalanceDisplay({
  balanceData,
  isBalanceLoading,
  isBalanceError,
  balanceError,
  ...
  if (isBalanceLoading) {
    return <Skeleton className="eb-h-6 eb-w-24" />;
  }
  if (isBalanceError) {
    return (
      <ServerErrorAlert
...
/**
 * Full-mode balance display
 */
function FullBalanceDisplay({
  balanceData,
  isBalanceLoading,
  isBalanceError,
  balanceError,
  ...
  if (!balanceData?.balanceTypes?.length) {
    return (
          ...
          >
            <div className="eb-flex eb-items-center eb-gap-1">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    aria-label={tString(
                      ...
                      {
                        type: t(`accounts:balanceTypes.${b.typeCode}`, {
                          defaultValue:
                            b.typeCode === 'ITAV'
                              ? 'Available Balance'
                              : 'Current Balance',
                ...
                <PopoverContent side="top" align="start" className="eb-w-72">
                  <div className="eb-space-y-1.5">
                    ...
                    <p className="eb-text-xs eb-text-muted-foreground">
                      {t(`accounts:balanceTypes.${b.typeCode}_description`, {
                        defaultValue:
                          b.typeCode === 'ITAV'
            ...
            <span className="eb-font-mono eb-break-words eb-text-xl eb-font-bold eb-leading-tight eb-text-metric @md:eb-text-2xl">
              ...
              <span className="eb-text-sm @md:eb-text-base">
                .{formatNumberWithCommas(Number(b.amount), locale).decimal}{' '}
                {balanceData.currency}
              </span>
            </span>
          </div>
      ...
      {balanceData.date && (
        <p className="eb-text-xs eb-text-muted-foreground">
          {t('accounts:card.dataAsOf', {
            date: new Date(`${balanceData.date}T00:00:00`).toLocaleDateString(
              locale,
              {
                year: 'numeric',
                month: 'short',
 ...
 * Compact card layout
 */
function CompactAccountCardView({
  account,
  className,
  Heading,
  displayName,
          ...
          <div className="eb-flex eb-flex-wrap eb-items-center eb-gap-x-2 eb-gap-y-0.5 eb-pt-1">
            ...
            <span className="eb-flex eb-items-center eb-gap-1">
              <span className="eb-font-mono eb-text-sm eb-font-medium eb-tracking-wide eb-text-foreground">
                {showSensitiveInfo ? fullAccountNumber : maskedAccountNumber}
              </span>
              <Button
                variant="link"
                size="sm"
...
export const AccountCard = forwardRef<AccountCardRef, AccountCardProps>(
  (
    ...
    const viewProps: AccountCardViewProps = {
      account,
      className,
      hideBorder,
      Heading,
    ...
    if (compact) {
      return <CompactAccountCardView {...viewProps} />;
    }

    // NORMAL MODE - Full card layout
    return <FullAccountCardView {...viewProps} />;
  }
);
