...
describe('Recipients Component', () => {
  ...
  describe('Microdeposit Verification', () => {
    test('shows verification button for microdeposit recipients', async () => {
      server.use(
        http.get('/recipients', () => {
          return HttpResponse.json({
            recipients: [mockMicrodepositRecipient],
            limit: 10,
            page: 1,
            total_items: 1,
          });
      ...
      renderComponent({ enableVerification: true });
    ...
    test('opens verification dialog when verify button is clicked', async () => {
      server.use(
        http.get('/recipients', () => {
          return HttpResponse.json({
            recipients: [mockMicrodepositRecipient],
      ...
      expect(screen.getByText('Microdeposit Verification')).toBeInTheDocument();
    });

    test('completes verification successfully', async () => {
      ...
      const readyButton = screen.getByText("I'm Ready to Verify");
      await userEvent.click(readyButton);

      const amount1Input = screen.getByLabelText('First Amount');
      await userEvent.type(amount1Input, '0.23');

      const amount2Input = screen.getByLabelText('Second Amount');
      await userEvent.type(amount2Input, '0.45');

      const verifyButton = screen.getByText('Verify Amounts');
    ...
    test('hides verification button when enableVerification is false', async () => {
      server.use(
        http.get('/recipients', () => {
          return HttpResponse.json({
            recipients: [mockMicrodepositRecipient],
      ...
      renderComponent({ enableVerification: false });

      await waitFor(() => {
        expect(screen.getByText('Jane Smith')).toBeInTheDocument();
      });

      const verifyButtons = screen.queryAllByRole('button', {
        name: /checkcircle/i,
      });
      expect(verifyButtons).toHaveLength(0);
    });
  });
