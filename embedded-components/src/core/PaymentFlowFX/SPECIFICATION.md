...
> **Date**: 2026-07-06 (rev 2 — verified against local OpenAPI specs)
...
6. Convert display amount using `fromBuyCcyToSellCcy` + `baseCcy` semantics: compute
   *target = USD × rate* or *USD ÷ rate* accordingly. Never assume multiply.
...
current recipients spec (1.0.47) lacks `currencyCode` and supports only `USABA` routing —
a newer recipients spec is required for FR-FX-10 (§4 step 3, OQ-2).
...
3. **Recipients spec upgrade — separate, gated step** for FR-FX-10: obtain the recipients
...
```ts
export const SUPPORTED_TARGET_CURRENCIES: string[] = [/* §3.4 minus USD */];
...
FX ⇒ only `FX_ALLOWED_METHODS`; RTP rendered disabled with reason via
...
|------|---------|-----------|
| `realtime` (default) | — | No fetch. With `getIndicativeRate`: fetch on `targetCurrency` change, show estimate labelled *indicative*. Without: show "Converted to {CUR} at the market rate when processed". Never submits `rateId`. |
...
Preview card content: "Recipient gets ≈ {Intl-formatted target amount}", "1 USD =
{rate} {CUR}", lock/indicative label, expiry countdown, `aria-live="polite"` on updates,
skeleton while fetching.

### FR-FX-7 — Review panel FX block (`fxSummary` prop)
Between amount row and totals: *You send $X USD* / *Recipient gets ≈ Y CUR (or
"determined at processing")* / *Exchange rate … (indicative | locked · expires in …)*.
...
- New "Recipient's bank country" select; US ⇒ exact current form.
- Non-US ⇒ `account.countryCode`; "Account currency" select (constrained to
...
- Gated on the recipients spec upgrade (§4 step 3) — ships in phase B4.
...
| B4 | Recipients spec upgrade (§4 step 3) + international recipient form (FR-FX-10/11) | FX recipient creation E2E |
...
3. **Currency/country badges** in the payee list (flag + ISO code) — destinations
...
| OQ-2 | Recipients spec version with `account.currencyCode` + international `routingCodeType` values not yet in repo. | FR-FX-10 gated on it (phase B4); until then tolerant accessor reads forward-compatible fields (§4 step 3). Exact routing enum confirmed from the regenerated schema before form work. |
