...
- **Payment flow variant (`paymentFlowVariant`)** — non-breaking; default `'domestic'`
  - `'domestic'`: Pay opens the standard PaymentFlow.
  - `'fx'`: Pay opens PaymentFlowFX for cross-border / multicurrency payouts; optional `fxConfig` and `supportedTargetCurrencies` are forwarded to that flow.
  - **Currency display**: when `paymentFlowVariant` is `'fx'`, recipient currency is shown by default (table column, card badges, details dialog) via effective `showRecipientCurrency`. Hosts may override `showRecipientCurrency` explicitly; otherwise domestic stays currency-hidden.
...
- **Table view**
  ...
  - When effective `showRecipientCurrency` is true (default for `paymentFlowVariant="fx"`): include a Currency column (ISO code / badge from `account.currencyCode`).
...
- **View details**
  ...
  - When currency display is enabled: show account currency in the detail view.
