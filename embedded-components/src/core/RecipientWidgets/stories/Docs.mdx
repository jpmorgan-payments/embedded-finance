...
> **Architecture Note:** This component is a thin wrapper around `BaseRecipientsWidget` configured with `recipientType="LINKED_ACCOUNT"`. For payment recipients without microdeposit verification, see [RecipientsWidget](?path=/docs/core-recipientswidget--docs).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        ✅ <strong>Microdeposit Verification</strong>
      </td>
      <td>
    ...
    <tr>
      <td>
        📊 <strong>User Event Tracking</strong>
      </td>
...
> 👉 **See the "Default" story in the sidebar for a live interactive demo.**
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Visual display style for accounts</td>
    </tr>
    <tr>
      <td>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Pagination style: incremental loading or traditional numbered pages
      </td>
    </tr>
    <tr>
      <td>
...
Same additive props as [RecipientsWidget](?path=/docs/core-recipientswidget--docs):
`paymentFlowVariant` (default `'domestic'`), auto `showRecipientCurrency` when FX,
plus optional `fxConfig` / `supportedTargetCurrencies`. See that docs page and the
**FX Payments** story for details.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        <code>(context) =&gt; void</code>
      </td>
      <td>
        Handler called for each user interaction event. Use for RUM/analytics
        integration
      </td>
    </tr>
    <tr>
      ...
      <td>
        Lifecycle hooks for action tracking (e.g., Dynatrace
...
<LinkedAccountWidget
  ...
  userEventsLifecycle={{
    onEnter: (context) => dtrum.enterAction(context.actionName),
    onLeave: (context) => dtrum.leaveAction(context.actionId),
  }}
/>
```

> 👉 **See the "User Journey Tracking" stories** for more examples including Dynatrace and Datadog integration.
...
> **💡 Instant Verification**: Some accounts may verify instantly and skip directly to `ACTIVE` status without microdeposits.
...
```tsx
import { PaymentFlow } from '@embedded-components';
  ...
  return (
    <>
      ...
      <PaymentFlow
        ...
        initialPayeeId={selectedRecipientId}
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Analytics integration examples (Dynatrace, Datadog, etc.)</td>
    </tr>
  </tbody>
</table>
...
- **[RecipientsWidget](?path=/docs/core-recipientswidget--docs)** - For payment recipients without microdeposit verification
...
> **⚠️ Migration Note**: If you're using `showCreateButton={false}`, change it to `hideCreateButton={true}` (the logic is inverted).
