...
export const OverviewScreen = () => {
  ...
  // useGetValidationMessage are hook-based. Passing the pre-built
  // `deltaStepSchemas` (assembled from the UNFILTERED step list) makes
  // getFlowProgress reuse those schemas via pure `safeParse` instead of calling
  // the schema hooks itself, so a step whose `isVisible` toggles can't change
  // the hook-call count between renders.
  ...
  return (
    <StepLayout
      title={
        isDeltaView
          ? t('screens.overview.deltaView.title', 'Finish up')
            ...
            <AlertDescription>
              {disclosureConfig?.platformName
                ? t('screens.overview.deltaView.partnerAlert', {
                    ...
                    defaultValue:
                      '{{partnerName}} has already provided most of your information. Just fill in the remaining items below to finish.',
                  })
                : t(
                    'screens.overview.deltaView.partnerAlertGeneric',
                    'Most of your information has already been provided by the platform. Just fill in the remaining items below to finish.'
                  )}
            </AlertDescription>
          </Alert>
    ...
    >
      <div className="eb-flex-auto eb-space-y-6">
        ...
        {overviewViewMode === 'full' && !isDeltaView && (
          <>
            {deltaAvailable && (
              <div className="eb-mt-6 eb-flex eb-items-center eb-justify-between eb-gap-4 eb-rounded-lg eb-border eb-border-primary/20 eb-bg-primary/5 eb-px-4 eb-py-3">
                <p className="eb-text-sm eb-text-foreground">
                  {t(
                    'screens.overview.deltaView.resumeBanner.description',
                    'A few details are needed before you can submit.'
                  )}
                </p>
                <Button
                  type="button"
