...
import type { ClientProductList, ClientQuestionResponse } from '@/api/generated/smbdo.schemas';
...
export function formatApplicationStatus(status: string | undefined): string {
  if (!status) return '—';
  return STATUS_LABELS[status] ?? status.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, (c) => c.toUpperCase());
}

export function formatProducts(products: ClientProductList | undefined): string {
  if (!products?.length) return '—';
  return products
    .map((p) => PRODUCT_LABELS[p] ?? p.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, (c) => c.toUpperCase()))
    .join(', ');
}
...
export function formatCustomerIdentityStatus(status: string | undefined): string {
  if (!status) return '—';
  return CUSTOMER_IDENTITY_STATUS_LABELS[status] ?? status.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, (c) => c.toUpperCase());
}
 ...
 */
export function formatQuestionResponseValue(response: ClientQuestionResponse): string {
...
export function formatRoleLabels(roles: string[] | undefined): string | undefined {
  if (!roles?.length) return undefined;
  return roles
    .map((r) => ROLE_LABELS[r] ?? r.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, (c) => c.toUpperCase()))
    .join(', ');
}
 ...
 */
export function formatJobTitleDisplay(individualDetails: {
  jobTitle?: string;
